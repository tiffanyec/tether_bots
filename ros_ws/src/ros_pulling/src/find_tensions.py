...
class Tensions():
    ...
    def __init__(self, des_angle, des_x_pos):
        ...
        self.right_car_pos = []
        self.block_all_pos = []
        # Times (these should probably be the same...)
        self.block_time = []
        self.ten_time = []
        self.pos_time = []
        ...
        # ROS subscribers
        rospy.Subscriber('/sim_ros_interface/block_angle', Float32MultiArray, self.block_angle_callback, queue_size=1, buff_size=2**8)
        rospy.Subscriber('/sim_ros_interface/pos', Float32MultiArray, self.pos_callback, queue_size = 1, buff_size=2**10)
        # rospy.Subscriber('/sim_ros_interface/block_pos', Float32MultiArray, self.pos2_callback, queue_size = 1, buff_size=2**8)
        # rospy.Subscriber('/sim_ros_interface/car_pos1', Float32MultiArray, self.pos1_callback, queue_size = 1, buff_size=2**8)
...
# 
    ...
    def pos_callback(self, msg):
        ...
        # Log views of position data. Note that these are views...
        # Also log time
        self.left_car_pos.append(car1_pos)
        self.right_car_pos.append(car2_pos)
        self.block_all_pos.append(self.block_pos[0])
    ...
    def car1_find_angle(self, car1_pos, block_pos):
        x = np.sqrt((self.diag/2)**2 - (block_pos[2])**2)
        left_dist = x - car1_pos[0]
        c = ((self.d*2)**2 + (self.rope1_len)**2 - left_dist**2) / (2*(self.d*2)*(self.rope1_len))
        return np.arccos(c)
    
    def car2_find_angle(self, block_pos):
        g = np.arcsin(block_pos[2] / (self.diag + self.rope2_len))
        return (np.pi/2) - g
...
if __name__ == '__main__':
    des_a = input("Please enter desired block angle (rad): ")
    des_p = input("Please enter desired block x position (m): ")
